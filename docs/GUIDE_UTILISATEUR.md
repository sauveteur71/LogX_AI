...
Le **manipulateur WinKeyer (K1EL)** — section dédiée dans la carte Radio — est la solution complète : branché sur son propre port série, il prend la main sur l'envoi CW **quelle que soit la marque de radio**, y compris Icom et Yaesu qui n'ont aucune manipulation native. La vitesse se règle dans LogX AI et non sur la radio. Le bouton **🔌 Tester le WinKeyer** ouvre une session avec le boîtier et affiche la version de son micrologiciel : c'est la seule preuve qu'un manipulateur est bien au bout du câble, un simple adaptateur USB répondrait sinon « port ouvert » sans jamais manipuler. Le WinKeyer a son propre processeur, donc une cadence qui ne dépend pas du trafic CAT — c'est pourquoi N1MM, Win-Test et DXLog le recommandent tous.
...
La section **🎚️ SO2R — DEUXIÈME RADIO** ne concerne que les stations à **deux radios**, où l'on appelle CQ sur une bande pendant qu'on cherche des multiplicateurs sur l'autre. Déclarez la seconde radio : **MODE DE PILOTAGE** — **Natif (série)** ou **OmniRig (composant COM Windows)** — puis **MARQUE**/**PORT SÉRIE**/**VITESSE** en natif, ou le **RIG OMNIRIG** (Rig1/Rig2, différent de celui choisi pour la radio 1) en OmniRig. Le QSY, les macros et le manipulateur visent alors **la radio qui a l'émission**, et la bascule se fait au clavier par **Ctrl+Espace**. Un indicateur `🎚 TX R1` apparaît en tête du band map — uniquement si une seconde radio est déclarée. **En pratique, préférez OmniRig pour la radio 2** : le CAT natif série ne garde qu'**une seule** connexion ouverte à la fois — basculer le focus vers la radio 2 ferme la liaison à la radio 1, donc **écouter les deux radios en même temps est aujourd'hui impossible en natif/natif**. OmniRig n'a pas cette limite.

La commutation **réelle** (quelle radio émet, ce que vous entendez dans quelle oreille) se fait dans un **boîtier matériel** : aucun logiciel ne route de l'audio à la place d'un relais. LogX AI lui dit quoi commuter via le protocole **OTRSP**, que comprennent les boîtiers microHAM, YCCC et EA4TX. En écoute **stéréo** — le mode de travail habituel — vous gardez une oreille sur la radio qui appelle pendant que vous cherchez de l'autre. Sans boîtier, la bascule reste logicielle : le pilotage suit la bonne radio, mais c'est à vous de rebrancher le casque.

Deux réglages audio complètent le SO2R. Le champ **PÉRIPHÉRIQUE VOCAL RADIO 2** (section SO2R) choisit la sortie audio du keyer vocal pour la radio 2 — laissé vide, il réutilise celui de la radio 1, à ne changer que si chaque radio a sa propre interface audio dédiée. Le **DÉCODEUR CW — RADIO 2**, sous le décodeur habituel dans le bandeau du bas, tourne en **parallèle** du premier (pas de bascule à faire) : à vous de choisir dans chaque panneau quel périphérique d'entrée correspond à quelle radio, LogX AI ne peut pas le déduire. Il n'apparaît que si une radio 2 est déclarée.

**⚠️ Ce que le SO2R de LogX AI ne garantit PAS.** Un verrou logiciel empêche d'armer une émission sur la radio 2 tant que la radio 1 émet encore (et réciproquement) — ça réduit le risque de double émission, disqualifiante dans la plupart des règlements de concours. Mais **rien dans LogX AI ne protège le récepteur d'une radio contre l'énergie RF de l'antenne voisine** quand l'autre émet, même en respectant ce verrou : c'est un risque physique, réel, qui se traite avec des **filtres passe-bande** entre chaque radio et son antenne — quasi obligatoires dès qu'on approche des puissances et antennes rapprochées d'un concours sérieux. Le dialogue OTRSP est conforme à la spécification du protocole mais n'a encore jamais été validé sur un vrai boîtier d'interlock. Ce qui **n'est pas** fait non plus : le « dueling CQ » automatique, les scénarios de commutation avancés et le second band map à l'écran.
...
**📠 DÉCODEUR RTTY.** Le RTTY s'ouvre depuis l'onglet **MODE NUMÉRIQUE** (à côté de FT8) dans sa **propre fenêtre détachable** — déplaçable où vous voulez, y compris sur un second écran. Il écoute l'audio de réception (câble virtuel ou interface dédiée depuis la radio, **pas le micro du PC**) et affiche le texte décodé — sans logiciel externe, là où N1MM s'appuie sur MMTTY, MMVARI ou Fldigi. Réglez **Mark** (2125 Hz par défaut) et le **Shift** (170 Hz, le standard amateur) ; restez à moins de 50 Hz du signal, au-delà le décodage se dégrade.

**Chaque indicatif décodé est cliquable** : un clic l'envoie dans la saisie de LOGBOOK. C'est ce geste qui fait la vitesse en RTTY — recopier à la main un indicatif déjà affiché est du temps perdu et une source de faute de frappe. Les mots ordinaires (CQ, TEST, DE) et les reports chiffrés ne sont pas cliquables, pour éviter les clics de travers. Comme la fenêtre est détachée, il faut l'ouvrir manuellement à chaque session (un navigateur ne peut pas ouvrir une fenêtre tout seul sans clic) — elle ne surgit plus automatiquement au passage en mode RTTY sur la radio.
...
LogX AI embarque un décodeur Morse temps réel qui tourne entièrement dans le navigateur, sans logiciel supplémentaire. Le panneau **🔤 DÉCODEUR CW** est fixé en bas à gauche de l'écran et **n'apparaît qu'en mode CW** — dans les autres modes il s'efface (en-tête cliquable pour l'ouvrir ou le replier ; la vitesse détectée en mots/minute s'affiche dans l'en-tête). Le RTTY, lui, s'ouvre à la demande dans sa propre fenêtre détachée (§6.5, onglet MODE NUMÉRIQUE) plutôt que d'apparaître automatiquement dans LOGBOOK.
...
Au même emplacement, le panneau **🖼 DÉCODEUR SSTV** n'apparaît qu'en **mode SSTV** : cochez d'abord **SSTV** dans CONFIG > MODES, puis choisissez SSTV dans le sélecteur de mode du logbook. Il reçoit les images à balayage lent — sorties portables, dimanches SSTV, et les transmissions de l'**ISS** (mode PD120) — sans MMSSTV ni RX-SSTV à côté. Mêmes prérequis audio que le décodeur CW (audio de réception de la radio, pas le micro du PC). Aucun réglage de mode : l'en-tête **VIS** du signal est détecté automatiquement — **23 modes** couverts (Martin M1 à M4, Scottie S1 à S4 et DX, Robot 36/72 en couleur et 8/12/24 en noir & blanc, PD50 à PD290, Wraase SC2-120/180) — et l'image se construit ligne par ligne sur l'écran, avec la progression en pourcentage dans l'en-tête du panneau. La dérive d'horloge entre cartes son (l'image « penchée » bien connue en SSTV) est compensée en continu sur les impulsions de synchro. **💾 PNG** télécharge l'image reçue (nom horodaté, pour enchaîner les réceptions sans rien écraser), **🗑** efface l'écran.
...
Le bouton **📥** produit automatiquement le bon format de soumission selon le concours actif — son libellé lui-même l'indique (« Exporter le log (Cabrillo) » ou « Exporter le log (EDI) »), rien à choisir : Cabrillo v3 pour tous les concours qui déposent en Cabrillo (CQ WW/WPX, ARRL DX, ARRL Field Day, championnats de France HF, WAE, UBA, European HF Championship, Russian DX, SP DX, HA DX, All Asian, Stew Perry, ARRL 10m/160m, REF 160m…), avec les noms officiels de concours, le score revendiqué et — pour le Field Day — le bloc classe/émetteurs ; EDI (REG1TEST) pour les concours VHF/UHF/SHF, avec l'en-tête complet (locator, section, club, opérateurs, matériel et antenne par bande).
...
Choisissez le périphérique d'entrée — **l'écoute démarre toute seule** dès qu'il est connu (le bouton ne sert plus qu'à couper puis reprendre). Si votre radio présente son entrée **et** sa sortie sous le même appareil (cas de l'IC-7300, « USB Audio CODEC » des deux côtés), choisir l'un **sélectionne l'autre automatiquement** — fini l'entrée qui tombe par erreur sur le micro du PC. Un **vu-mètre de niveau RX** (barre 0-80 dB, repère WSJT-X) vous aide à régler le volume d'entrée dans Windows : trop bas (orange) rien ne décode, saturé (rouge) le décodage se dégrade, visez le vert. Vous obtenez une chute d'eau et le tableau des décodages : heure UTC, fréquence en Hz, **DT** (le décalage temporel de la station d'en face — le seul moyen de voir d'un coup d'œil si c'est son horloge ou la vôtre qui dérive), **SNR en dB**, et le message. La liste **CQ ENTENDUS** (à gauche) affiche le **SNR** de chaque station, code couleur par force du signal — de quoi choisir qui appeler.

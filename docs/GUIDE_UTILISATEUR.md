...
Le panneau « 🎙 KEYER VOCAL — clic: jouer · ⏺: enregistrer » remplace les macros quand le mode n'est pas CW. Quatre slots : **CQ**, **RÉPONSE**, **REPORT**, **MERCI**. Le bouton ⏺ enregistre au micro de votre PC (re-clic pour arrêter), ▶ rejoue le message. Deux limites à connaître : la lecture se fait **par les haut-parleurs du navigateur** (pas de PTT automatique — c'est à vous d'acheminer l'audio vers la radio), et les enregistrements sont stockés **dans ce navigateur uniquement** : ils ne suivent pas d'un PC à l'autre.
...
- **Macros CW** (panneau **MACROS — clic: copier · double-clic: modifier**) : 8 macros par défaut (« CQ RPH », « ÉCHANGE », « TU », « QSY 432? », « LOCATOR », « ? », « AGN? », « 73 »), avec variables {CALL} (votre propre indicatif ici), {LOC} et {NR} remplacées à la volée. Si la radio est en CW et le CAT actif, le texte part directement au manipulateur de la radio (toast « 📻 CW → … ») ; sinon il est copié dans le presse-papier (« 📋 … »). Attention : les macros se déclenchent **au clic** — les touches physiques F1 à F8 ne sont pas interceptées (seule F9 soumet le QSO).

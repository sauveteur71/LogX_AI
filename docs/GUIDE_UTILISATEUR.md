...
La section **🎚️ SO2R — DEUXIÈME RADIO** ne concerne que les stations à **deux radios**, où l'on appelle CQ sur une bande pendant qu'on cherche des multiplicateurs sur l'autre. Déclarez la seconde radio (marque, port, vitesse) : le QSY, les macros et le manipulateur visent alors **la radio qui a l'émission**, et la bascule se fait au clavier par **Ctrl+Espace**. Un indicateur `🎚 TX R1` apparaît en tête du band map — uniquement si une seconde radio est déclarée.
...
Ce qui **n'est pas** fait : le « dueling CQ » automatique, les scénarios de commutation avancés et le second band map à l'écran.

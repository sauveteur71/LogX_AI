...
LogX AI embarque un décodeur Morse temps réel qui tourne entièrement dans le navigateur, sans logiciel supplémentaire. Le panneau **🔤 DÉCODEUR CW** est fixé en bas à gauche de l'écran et **n'apparaît qu'en mode CW** — dans les autres modes il s'efface, comme le décodeur RTTY n'apparaît qu'en RTTY (en-tête cliquable pour l'ouvrir ou le replier ; la vitesse détectée en mots/minute s'affiche dans l'en-tête).

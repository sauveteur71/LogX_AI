...
| #287 | **Panadapter : peak hold** (maintien de crête, décroissance lente). |
| #288 | **CHASSE #5 : crédit « carré neuf » VHF/UHF.** Au-dessus de 30 MHz, un carré Maidenhead jamais travaillé à vie vaut la chasse (VUCC). `evaluer_grid` pur (poids `new_grid`=450, objectif `vucc`) ; `annoter_credit` bâtit l'index carrés-à-vie dans SON scan unique et **combine au crédit DXCC par le meilleur score** (les poids F4GLD tranchent : ATNO 1000/+BANDE 600 priment). `/data/focus` porte enfin le locator. Badge 🗺 +CARRÉ. **Au passage** : borne du regex `_handler` du test de câblage corrigée (12→8 espaces) — elle sur-capturait le handler suivant et rendait les asserts `locator` **vacants** (piège consigné en mémoire, détecté par contre-épreuve). |

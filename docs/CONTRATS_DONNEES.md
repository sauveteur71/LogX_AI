...
- **1.3.0** (12/09/2026) — nouvelle forme de points `{'per_km_x': N}` (`properties.scoring.properties.bricks.properties.points.items.properties.points.oneOf`) : distance en km × N, motivée par les 4 concours REF TVA (barème 2/4/10 pts/km selon la bande, découvert en lisant les règlements officiels — l'ancienne donnée d'affichage `CONTEST_SCORING` prétendait à tort un multiplicateur « relais TVA » inexistant dans les règlements). MINOR : forme additive, aucune définition existante n'a besoin d'être modifiée. Au passage, un trou PRÉEXISTANT du vérificateur minimal de secours (`logx_validate.py::_check_scoring`) a été corrigé : il n'acceptait ni `'per_km_stew'` ni `'per_grid_3000'` en forme `bricks` directe (seulement via `param`/nombre/`'per_km'`) — resté invisible jusqu'ici car le seul autre concours à utiliser `'per_grid_3000'` (`FT_CHALLENGE`) passe par la forme raccourcie `'type'`, qui ne traverse pas cette branche de code. Corrigé en miroir du schéma JSON réel, avec témoin vert + contre-épreuve par mutation dédiés (`tests/test_contest_ref_tva.py`).
- **1.4.0 / 1.5.0** — bumps effectués (Challenge THF puis Rencontres UFT, cf. `docs/passation/PASSATION.md` 13/09/2026) sans que cette page soit mise à jour en même temps — gap constaté le 17/09/2026, laissé tel quel (backfill hors du périmètre du chantier qui l'a découvert) plutôt que reconstitué de mémoire.
- **1.6.0** (17/09/2026) — nouvelle valeur de `multiplier.kind` : `ha_county_dxcc` (`properties.scoring.properties.bricks.properties.multiplier.properties.kind.enum`), et nouvelle valeur de prédicat `when` : `is_ha` (les deux occurrences, forme chaîne et forme tableau combiné). Motivé par le HA-DX Contest (MRASZ Hongrie) : la définition `CONTEST_DEFINITIONS['HA_DX']` portait un barème et un multiplicateur PLACEHOLDER (« approximatif », `zone_dxcc` générique) alors que le vrai moteur dédié (`_mult_ha_county_dxcc`, `_ha_county_token`) existait déjà côté `logx_scoring.py` mais n'était référencé par aucune définition. MINOR : forme additive, aucune définition existante n'a besoin d'être modifiée. Témoin vert + contre-épreuve par mutation dédiés (`tests/test_contest_ha_dx.py`).

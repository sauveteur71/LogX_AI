...
> **CORRIGÉ le 21/08/2026.** Les deux volets confirmés exacts.
> — **Auth** : `/log/list` (logx_http.py:2234) n'appelait `_require_auth()` nulle part dans son corps, contrairement à `/debug/errors` juste au-dessus ou `/log/next_serial` juste en dessous — renvoyait le carnet entier (indicatifs, RST, commentaires) à quiconque atteignait le port. Corrigé (`if not self._require_auth(): return` en tête de bloc). Sans danger pour l'usage navigateur normal : chaque page `.html` pose déjà le cookie `rc_token` à son chargement (`do_GET`, quand aucun mot de passe d'accès n'est configuré) — `/log/list` n'est jamais appelée avant qu'une page ait fini de charger. A cassé 6 fichiers de test qui appelaient la route sans jeton (habitude héritée de l'ancien comportement non authentifié) : corrigés pour envoyer `X-RC-Token`.
> — **SRI** : 6 balises `<script src="https://...">`/`<link href="https://...css">` externes (Leaflet ×5 pages, Chart.js) sans `integrity`/`crossorigin`, alors que `logx_wall.html` avait déjà le bon motif pour Leaflet. Hachés calculés en téléchargeant réellement chaque fichier (`sha512`, `openssl dgst`) — jamais recopiés à l'aveugle : l'URL cdnjs de `logx_wall.html` a été réutilisée seulement là où l'URL est identique (mêmes octets garantis), l'URL unpkg (différente, non minifiée) a eu son propre calcul. `chart.js@4` (version flottante) épinglé à la version réellement résolue au moment du correctif (`4.5.1`, via l'API `data.jsdelivr.com`) — indispensable avec SRI, sinon le hash casse au prochain patch publié.
> Garde-fous ajoutés : `tests/test_log_delta_sync.py` (3 tests : sans jeton → 403, avec `X-RC-Token` → 200, avec cookie `rc_token` → 200) et `tests/test_sri_cdn_externe.py` (toute balise CDN externe future doit porter `integrity`+`crossorigin`, sur les 19 pages HTML). Contre-épreuves par mutation faites sur les deux (rouge confirmé, restauré, vert reconfirmé). Suite complète (~10 000 tests) revérifiée verte après le correctif d'authentification.

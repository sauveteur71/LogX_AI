/* Service worker RadioContest AI — installabilité (PWA) + shell hors-ligne.
...
const CACHE = 'radiocontest-v1';
const SHELL = [
  '/radiocontest_logbook.html', '/radiocontest_logbook.js',
  '/radiocontest_statusbar.js', '/radiocontest_i18n.js',
  '/radiocontest_icon.svg', '/manifest.webmanifest',
];
...
self.addEventListener('fetch', e => {
  ...
  e.respondWith(fetch(req).then(res => {
    ...
    return res;
  }).catch(() => caches.match(req).then(hit => hit || caches.match('/radiocontest_logbook.html'))));
});

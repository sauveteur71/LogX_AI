...
complet (radiocontest.db + shared_log.json + un export ADIF ré-importable)
...
def run_backup(cfg, shared_log=None):
    ...
    base = f'radiocontest_{_safe(call)}_{stamp}'
    written = []
    try:
        # 1) SQLite (source de vérité) si présent
        if os.path.isfile('radiocontest.db'):
            dst = os.path.join(folder, base + '.db')
            shutil.copy2('radiocontest.db', dst)
...
def _prune(folder):
    ...
    try:
        bases = sorted({os.path.basename(p).rsplit('.', 1)[0]
                        for p in glob.glob(os.path.join(folder, 'radiocontest_*'))})
...
def status(cfg=None):
    ...
    if s.get('folder') and os.path.isdir(s['folder']):
        count = len(glob.glob(os.path.join(s['folder'], 'radiocontest_*.db')))

...
Complète radiocontest_rig.py (rigctld/Hamlib) plutôt que le remplacer : ce
...
def cat_settings(cfg):
    ...
    'native' (ce module) de 'rigctld' (radiocontest_rig, inchangé) — une
...
def get_state(cfg):
    """État courant (fréquence + mode) — même forme que radiocontest_rig
...
def set_freq(cfg, freq_hz, mode=None):
    """QSY natif — même signature que radiocontest_rig.set_freq."""
    settings = cat_settings(cfg)
    if not settings['enabled'] or settings['mode'] != 'native':

...
# EV-7 49e increment : _reserveBottomSpace (appele par renderPartnerTyping)
# extrait vers logx_panel_layout.js -- charge avant logbook.js, portee globale.
...
def _real_source(rev=None):
    ...
    if rev is None:
        ...
        with open(CLOCK_JS_PATH, encoding='utf-8') as f:
            src += '\n' + f.read()
        with open(PANEL_LAYOUT_JS_PATH, encoding='utf-8') as f:
            src += '\n' + f.read()

...
constructeurs vérifiés contre leur documentation officielle :
  ...
  - Icom IC-PW2/PW-1 : CI-V étendu (IC-PW2 CI-V Reference Guide) — réutilise
    l'encodage/décodage de trame CI-V de radiocontest_cat.py (même FE FE.../FD).
...
Comme radiocontest_cat.py : l'encodage/décodage de trame est séparé du
...
from radiocontest_cat import (HAS_PYSERIAL, SerialPort, list_ports,
...
# ═══════════════════════════════════════════════════════════════════════════
#  Icom IC-PW2 / PW-1 — CI-V étendu (réutilise le codec de radiocontest_cat)
# ═══════════════════════════════════════════════════════════════════════════
...
class IcomAmp:
    ...
    def _raw2(data):
        ...
        même principe que get_smeter() de radiocontest_cat.py — valeur BRUTE,
...
# ═══════════════════════════════════════════════════════════════════════════
#  Réglages + connexion persistante (même schéma que radiocontest_cat.py)
# ═══════════════════════════════════════════════════════════════════════════
...
def _ensure_connected(settings):
    ...
    que radiocontest_cat._ensure_connected()."""
...
def set_operate(cfg, on):
    """Bascule standby/operate — même signature que radiocontest_cat.set_ptt,
...
def set_band(cfg, band_mhz):
    """QSY de bande de l'ampli — même esprit que radiocontest_cat.set_freq."""
    settings = amp_settings(cfg)
    if not settings['enabled']:
        return {'ok': False, 'error': 'Pilotage ampli désactivé (CONFIG)'}
    driver, err = _ensure_connected(settings)

...
const CONFIG_HELP = {
  ...
  cat_power_digital_w: "Puissance (en watts) poussée vers la radio dès que tu choisis un mode numérique (FT8/FT4/RTTY/PSK...) dans LOGBOOK, ou dès que tu actives l'émission sur la page FT8 — typiquement inférieure à la puissance phonie/CW pour protéger le final. Laisse vide pour désactiver la réduction automatique.",
  ...
  winkeyer_variant: "Variante du protocole. « WinKeyer 3 (K1EL) » applique tous les réglages fins (weighting, ratio, Farnsworth, sidetone, PTT). « WinKeyer 1-2 / K3NG » n'envoie que la vitesse (séquence de base sûre) — à choisir pour un keyer Arduino K3NG en émulation WinKeyer, ou un ancien boîtier WK1/WK2, qui pourraient ne pas comprendre les commandes WK3.",
  ...
  winkeyer_weighting: "Poids des éléments CW en % (10–90, 50 = neutre). En dessous de 50 les éléments sont plus courts, au-dessus plus longs ; la vitesse globale reste tenue par compensation des espaces. Réglages appliqués à la (re)connexion du WinKeyer.",
  winkeyer_ratio: "Ratio point/tiret (33–66, 50 = 1:3 standard). Toute autre valeur déforme volontairement le Morse — à laisser à 50 sauf besoin précis.",
  winkeyer_farnsworth: "Farnsworth : vitesse des caractères en mots/min (10–99), 0 = désactivé. Les lettres sont formées à cette vitesse, les espaces élargis pour tenir la vitesse globale (utile à l'apprentissage). Sans effet si supérieure à la vitesse normale.",
  winkeyer_sidetone_hz: "Fréquence du sidetone du WinKeyer (500–4000 Hz, WK3). N'affecte que la tonalité de contrôle du boîtier, pas la fréquence d'émission.",
  winkeyer_ptt_lead_ms: "Délai PTT avant le premier élément CW, en ms (0–2500, par pas de 10). Laisse le temps à l'ampli/relais de basculer avant que la porteuse parte.",
  winkeyer_ptt_tail_ms: "Délai PTT ajouté après le dernier élément, en ms (0–2500, par pas de 10). Le WinKeyer garde de toute façon au minimum 3 temps de point après le texte.",
  cw_serial_enabled: "Active le keyer CW par ligne série DTR/RTS (interface transistor/opto simple) — repli de manipulation quand aucun WinKeyer n'est branché. Si un WinKeyer est activé, il garde la priorité. Le timing des points/tirets est généré par l'ordinateur.",
...
function saveConfig(silent = false, feedbackBtn = null) {
  ...
  const config = {
    ...
    winkeyer_variant: document.getElementById('winkeyer_variant')?.value || 'WK3',
    winkeyer_port: document.getElementById('winkeyer_port')?.value.trim() || '',
    winkeyer_wpm: _numClamped('winkeyer_wpm', 25),
    winkeyer_weighting: _numClamped('winkeyer_weighting', 50),
    winkeyer_ratio: _numClamped('winkeyer_ratio', 50),
    winkeyer_farnsworth: _numClamped('winkeyer_farnsworth', 0),
    winkeyer_sidetone_hz: _numClamped('winkeyer_sidetone_hz', 800),
    winkeyer_ptt_lead_ms: _numClamped('winkeyer_ptt_lead_ms', 0),
    winkeyer_ptt_tail_ms: _numClamped('winkeyer_ptt_tail_ms', 0),
    cw_serial_enabled: document.getElementById('cw_serial_enabled')?.value || '',
...
function applyFullConfigToForm(c) {
    ...
    ['usage_mode','eqsl_user','eqsl_password','lotw_user','lotw_password','lotw_station_location','clublog_email',
     ...
     'winkeyer_enabled','winkeyer_variant','winkeyer_port','winkeyer_wpm',
